...
SLAPrint::write_svg(const std::string &outputfile) const
{
    ...
    for (size_t i = 0; i < this->layers.size(); ++i) {
        ...
        fprintf(f,
            "\t<g id=\"layer_%08u\" slic3r:z=\"%0.4f\" slic3r:slice-z=\"%0.4f\" slic3r:layer-height=\"%0.4f\">\n",
            i,
            layer.print_z,
            layer.slice_z,
        ...
        if (layer.solid) {
            ...
            for (ExPolygons::const_iterator it = slices.begin(); it != slices.end(); ++it) {
                ...
                int color= 0; //250 * i / this->layers.size()  ;
                fprintf(f,"\t\t<path d=\"%s\" style=\"fill: #%02x%02x%02x; stroke: #%02x%02x%02x; stroke-width: %s; fill-type: evenodd\" slic3r:area=\"%0.4f\" />\n",
                    pd.c_str(), color,color,color, color,color,color, "0", unscale(unscale(it->area()))
                );
            }
        } else {
            // Perimeters.
        ...
        if (i >= (size_t)this->config.raft_layers) {
            ...
            for (std::vector<SupportPillar>::const_iterator it = this->sm_pillars.begin(); it != this->sm_pillars.end(); ++it) {
                ...
                fprintf(f,"\t\t<circle cx=\"%f\" cy=\"%f\" r=\"%f\" stroke-width=\"0\" fill=\"yellow\" slic3r:type=\"support\" />\n",
...
void
SLAPrint::write_svg_layers(const std::string &outputfile) const
{
    const Sizef3 size = this->bb.size();
    const double support_material_radius = sm_pillars_radius();
	std::string  curfile;
    ...
    for (size_t i = 0; i < this->layers.size(); ++i) {
		...
		sprintf(fname,"%s.%08u.svg",outputfile.c_str(),i);
        ...
        if (layer.solid) {
            ...
            for (ExPolygons::const_iterator it = slices.begin(); it != slices.end(); ++it) {
                ...
                fprintf(f,"\t\t<path d=\"%s\" style=\"fill: %s; stroke: %s; stroke-width: %s; fill-type: evenodd\" slic3r:area=\"%0.4f\" />\n",
                    pd.c_str(), "black", "black", "1", unscale(unscale(it->area()))
                );
            }
        } else {
            // Perimeters.
