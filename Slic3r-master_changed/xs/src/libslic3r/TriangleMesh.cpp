    ...
    const std::vector<float> &z) const
{
    ...
    min_layer = std::lower_bound(z.begin(), z.end()-1, min_z); // first layer whose slice_z is >= min_z
	...
	if (min_z == max_z) {
		...
		max_layer = std::upper_bound(z.begin(), z.end()-1, max_z) ; // last layer whose slice_z is <= max_z
	}
    #ifdef SLIC3R_DEBUG
	...
	if (max_layer != min_layer) {
		...
		if (min_layer == z.end()) {
			printf("out of rabge  min_layer");
		}
		else {
			std::vector<float>::size_type layer_idx = min_layer - z.begin();
			this->slice_facet(*min_layer / SCALING_FACTOR, facet, facet_idx, min_z, max_z, &(*lines)[layer_idx], lines_mutex);
		}
	}
